...
alpha_plot_mtest <- function(data,design,seed=123,width=10,height=10,row_panel=NULL,method='LSD',group_level=c('default'),mytheme=theme(),palette=c("#E64B35FF","#4DBBD5FF","#00A087FF","#3C5488FF","#F39B7FFF","#8491B4FF",
  ...
  # 主要这里使用全局关闭警告再打开，避免输出过程中产生不必要的警告提示
  options(warn=-1)
  alpha_result_letter_for_total <- reshape2::melt(alpha_result_letter,id.vars = c('Group'))
  options(warn=1) 
...
alpha_plot=function(dir,group_level=c('default'),seed=123,mytheme=theme(),min_relative = 0,min_ratio = 0,design ,pattern = '',output = F,html_out = F,
