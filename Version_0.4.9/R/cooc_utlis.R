...
cooc_plot <- function(dir,min_relative = 0,min_ratio = 0,pattern='L',design,cooc_method='spearman',cooc_p=0.05,cooc_r=0.3,vertex.size=NULL,vertex.label.cex=NULL,edge.width=2,edge_color_positive='darkred',edge_color_negitive='steelblue',output = F,cooc_output = F,width=10,height=10,set_color_level='phylum'){
  deposit=list()
  # 注意这里adjust必须为关闭
  mapping <- design
  filter_result <- data_filter(dir = dir,min_relative = min_relative,min_ratio = min_ratio,design = design,adjust = F,pattern = pattern,output = output)
  ...
  for (Group_name in unique(mapping$Group)) {
    ...
    input_list <- data_filter(dir = dir,min_relative = min_relative,min_ratio = min_ratio,
    ...
    for (k in input_file){
      ...
      if (cooc_output ==T ) {
        ...
        dev.off() 
        fs::file_move(paste0(Group_name,'_',k,'.pdf'),Group_name)
      }
    }
  }
